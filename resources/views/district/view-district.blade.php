								...
								<div class="col-md-6">
									<div class="box-uploader">
										 <form action="{{ route('district-import') }}" method="POST" enctype="multipart/form-data">
											...
											<button class="btn btn-info btn-md">Import district</button>
										</form>
									</div>
								</div>
								<div class="col-md-6">
									 <button type="submit" class="btn btn-dark btn-right"
                                		onclick="window.location='{{ url('create-district') }}'">Add District
                            		</button>
								</div>
							</div>
                        </div>
                                    ...
                                    @if (isset($district))
                                        @foreach ($district as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ url('/district/' . $item->id) }}"
                                                        title="View district"><button class="btn btn-success btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                                    <a href="{{ url('/district/' . $item->id . '/edit') }}"
                                                        title="Edit district">
                                                        <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt" aria-hidden="true"></i>
                                                        </button>
                                                    </a>

                                                    <form method="POST" action="{{ url('/district' . '/' . $item->id) }}"
