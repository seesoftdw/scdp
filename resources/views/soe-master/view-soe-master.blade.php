...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">SOE master List</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Master</a></li>
                        <li class="breadcrumb-item active">SOE master List</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                    ...
                    <div class="card">
						<div class="card-header">
                           ...
                           @if (auth()->user()->role_id == '1')
                                <form action="{{ route('soe-master-import') }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    <input type="file" name="file" id="customFile">
                                    <button class="btn btn-info btn-md">Import SOE</button>
                                </form>
                            @endif
							</div>
							</div>
							<div class="col-md-6">
                             <button type="submit" class="btn btn-dark btn-right"
                                onclick="window.location='{{ url('create-soe-master') }}'">Add Soe master
                            </button>
                        </div>
						</div>
						</div>
                        ...
                        <div class="card-body">
                            ...
                            <table id="datatable" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @if (isset($soe))
                                        @foreach ($soe as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->soe_name }}</td>
                                                <td>
                                                    <a href="{{ url('/soe-master/' . $item->id) }}"
                                                        title="View soe"><button class="btn btn-success btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                                    <a href="{{ url('edit-soe-master?' . $item->id) }}" title="Edit soe">
                                                        <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt" aria-hidden="true"></i>
                                                        </button>
                                                    </a>

                                                    <form method="POST"
                                                        action="{{ url('/soe-master' . '/' . $item->id) }}"
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="6" class="text-danger" style="text-align: center">No record
                                                found</td>
                                        </tr>
                                    @endif
                                </tbody>
